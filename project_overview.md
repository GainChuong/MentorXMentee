...
│   │   └── static/                  # Chứa toàn bộ tài nguyên JS, CSS và Media (ảnh đã gán mã Hash)
...
│   ├── public/                      # Thư mục công khai chuẩn (chỉ chứa tệp khung mẫu HTML & SEO)
│   │   ├── index.html               # Tệp khung mẫu duy nhất của React SPA
│   │   ├── robots.txt               # Cấu hình robot chỉ mục tìm kiếm (SEO)
│   │   └── Cựu SV.code-workspace    # Cấu hình không gian làm việc VS Code
│   └── src/
...
│       ├── assets/                  # NƠI LƯU TRỮ DUY NHẤT TOÀN BỘ HÌNH ẢNH, LOGO, BANNER (Nén qua Webpack)
│       ├── context/
...
### ⚙️ A. Nhóm Khởi Chạy, Cấu Hình & Quản Lý Tài Nguyên (Asset Architecture)

| Tệp tin / Thư mục | Đường dẫn | Chức năng & Vai trò nòng cốt |
| :--- | :--- | :--- |
| **`src/assets/`** | `react-app/src/assets/` | **Nơi lưu trữ tập trung 100% hình ảnh & media của dự án.** Tất cả ảnh (Logo UEL, Banner, Avatar, Thumbnail) đều được `import` trực tiếp trong code JS. Webpack sẽ tự động tối ưu hóa, nén ảnh và gán mã Hash (gửi vào `build/static/media/`) giúp chống cache cũ. |
| **`public/`** | `react-app/public/` | **Thư mục công khai đã tinh gọn.** Chỉ giữ lại `index.html` (trang HTML mẫu bắt buộc của React SPA) và `robots.txt` (SEO). Không lưu trữ ảnh rác. |
...
2. **Biên dịch Production Build:** 
   ...
   npm run build
   ```
   *Lưu ý: Quá trình build sẽ tự động gom toàn bộ ảnh từ `src/assets/` sang `build/static/media/` kèm mã hash và kích hoạt `node patch_build_paths.js` để vá lại đường dẫn.*
...
*Tài liệu được cập nhật tự động & chuẩn hóa theo kiến trúc tài nguyên tập trung mới của dự án MentorXMentee - Khoa HTTT UEL.*
